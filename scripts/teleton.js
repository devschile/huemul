  ...
  robot.respond(/teleton/i, async msg => {
    try {
      ...
      if (!response.ok) {
        ...
        return msg.reply('ocurrió un error al obtener el cómputo de la Teletón')
      }
      ...
      const formattedCifra = `$${String(cifra).replace(/^\$+\s*/, '')}`

      // Usar la hora actual
      const time = moment()

      // Formatear según la especificación: Cómputo a las [HH:MM] [DD/MM/YYYY]: [CIFRA]
      const formatted = `Cómputo a las ${time.format('HH:mm')} ${time.format('DD/MM/YYYY')}: ${formattedCifra}`

      msg.send(formatted)
    } catch (err) {
      robot.emit('error', err, msg, 'teleton')
      msg.reply('ocurrió un error al obtener el cómputo de la Teletón')
    }
  })
}
